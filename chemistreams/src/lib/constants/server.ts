// database message types (each must be unique!)
export const YOUTUBE_EMBED_TYPE_CODE: number = 6
export const SPOTIFY_EMBED_TYPE_CODE: number = 5
export const REMOVE_USER_TYPE_CODE: number = 3
export const USER_LEFT_TYPE_CODE: number = 2
export const ADD_USER_TYPE_CODE: number = 1
export const MESSAGE_TYPE_CODE: number = 0
export const FILE_TYPE_CODE: number = 4

// in MB
export const MAXIMUM_IMAGE_UPLOAD_SIZE: number = 7
export const MAXIMUM_PFP_FILE_SIZE: number = 3
