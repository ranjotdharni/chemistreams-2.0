...
export default function ChatView({ current, editChat, chatList, setCurrentChat } : ChatViewProps) {
    ...
    const handleNewMessage = useCallback(async (snapshot: DataSnapshot) => {
        ...
        if (snapshot.key) {
            ...
            else if (rawMessageData.type === SPOTIFY_EMBED_TYPE_CODE) {
                message = {
                    id: messageId,
                    sender: rawMessageData.sender,
                    type: rawMessageData.type,
                    spotifyId: rawMessageData.spotifyId,
                    resourceType: rawMessageData.resourceType,
                    timestamp: new Date(rawMessageData.timestamp)
                }
            }
            else if (rawMessageData.type === FILE_TYPE_CODE) {
                message = {
                    id: messageId,
                    sender: rawMessageData.sender,
                    type: rawMessageData.type,
                    fileId: rawMessageData.fileId,
                    link: rawMessageData.link,
                    space: rawMessageData.space,
                    timestamp: new Date(rawMessageData.timestamp)
                }
            }
